...
export const processUnstoredImages = action({
  ...
  handler: async (ctx, { limit = 10, delaySeconds = 5 }) => {
    // Get unstored images
    const images: Doc<'images'>[] = await ctx.runQuery(internal.civitai.images.getUnstoredImages, { limit })
    ...
    if (images.length > 0) {
      ...
      await ctx.scheduler.runAfter(delaySeconds, api.civitai.images.processUnstoredImages, { limit })
    }
